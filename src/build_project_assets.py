...
profiles = {
    'good': {
        'positive': lambda: norm.rvs(loc=norm.rvs(4),
                                     scale=1).astype(int),
    ...
    'normal': {
        'positive': lambda: norm.rvs(loc=norm.rvs(3),
                                     scale=1).astype(int),
    ...
    'chaotic_good': {
        ...
        'negative': lambda: np.random.choice([0, np.random.choice(
            [50, 200])], p=[.98, .02]),
        'chance': .2
    },
    'chotic_bad': {
...
df = pd.DataFrame(data, columns=['employee_id',
                                 'team_id',
                                 'event_date',
                                 'positive_events',
                                 'negative_events',
...
notes = pd.DataFrame(_, columns=['employee_id',
                                 'employee_name',
                                 'note']).assign(
...
events = df[['event_date',
             'employee_id',
             'team_id',
             'positive_events',
             'negative_events']]

team = df.drop_duplicates('team_id')[['team_id',
                                      'team_name',
                                      'shift',
                                      'manager_name']]

notes = df.dropna()[['employee_id',
                     'team_id',
                     'note',
                     'event_date']].rename(
